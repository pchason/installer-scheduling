...
const schemaDescriptions = [
  ...
  'job_schedules.schedule_id (serial, PRIMARY KEY) - Unique identifier for a job\'s calendar schedule entry',
  'job_schedules.job_id (integer, FOREIGN KEY) - References `jobs.job_id`. Links a scheduled job to its details in the jobs table.',
  'job_schedules.scheduled_date (date) - The scheduled date for the job\'s work to begin',
  'job_schedules.status (enum: scheduled, completed, cancelled) - Status of the schedule entry for the job',

  'Table: installer_assignments - Assignments of installers to scheduled jobs based on an installer\'s trade specialty. Fields: assignment_id, schedule_id, installer_id, po_id, assignment_status',
  'installer_assignments.assignment_id (serial, PRIMARY KEY) - Unique identifier of the assignment',
  'installer_assignments.schedule_id (integer, FOREIGN KEY) - References `job_schedules.schedule_id`. Links to the schedule entry for a job.',
